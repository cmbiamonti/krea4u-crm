// src/components/Templates/DocumentEditor.tsx
...
import type {
  ...
  TemplateStatus, CreateDocumentDTO, UpdateDocumentDTO,
} from '@/types/template.types'
import jsPDF from 'jspdf'
import { generateDocumentPDF } from '@/components/templates/TemplateExport'
...
const DocumentEditor: React.FC<DocumentEditorProps> = ({
  ...
  useEffect(() => {
    if (document) {
      setFormData({
        ...
        data:        document.data || {},
        // ✅ metadata RIMOSSO — non esiste nel DB
      })
      setTemplateContent(
        (document as any).template_content ?? null
      )
    } else if (template) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (document) {
        // ── Aggiornamento ──────────────────────────────────────────────────
        // ✅ UpdateDocumentDTO NON ha metadata
        ...
        // ── Creazione ──────────────────────────────────────────────────────
        // ✅ CreateDocumentDTO NON ha metadata
        // template_name e template_version vengono recuperati in createDocument()
        const createData: CreateDocumentDTO = {
          template_id:      formData.template_id,
          name:             formData.name,
          status:           formData.status,
          data:             formData.data,
          // Passa subito nome e versione per evitare una query extra
          template_name:    template?.name    || '',
          template_version: template?.version || '1.0',
        }

        console.log('📤 DocumentEditor — createData:', createData)
  ...
  const handleExportPDF = async () => {
    try {
      ...
      // Costruisce un TemplateDocument temporaneo con i dati attuali del form
      const tempDoc = document
        ? {
            ...
            name:   formData.name,
            status: formData.status,
            // Inietta il template content se disponibile
        ...
        : {
            ...
            user_id:     '',
            metadata:    null,
            // Inietta il template content direttamente
            template_content: templateContent ?? undefined,
          }
      ...
      toast.error(`Errore export PDF: ${err.message}`)
    }
  }
      ...
      case 'textarea':
        return (
          <div key={field.id} className="space-y-2">
            ...
            <Textarea
              id={field.id} value={value} rows={4}
      ...
      case 'phone':
        return (
          <div key={field.id} className="space-y-2">
            ...
            <Input
              id={field.id} type="tel" value={value}
              onChange={(e) => handleFieldChange(field.id, e.target.value)}
              placeholder={field.placeholder} required={field.required}
            />
          </div>
      ...
      case 'checkbox':
        return (
          <div key={field.id} className="flex items-center space-x-2">
            <Checkbox
              id={field.id} checked={value === true}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="edit" className="space-y-4 mt-4">
          <div className="space-y-2">
            ...
            <Input
              id="docName" value={formData.name}
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Stato: {
                  ...
                  formData.status === 'signed'    ? 'Firmato'    : 'Archiviato'
                }
              </p>
            </div>
            <Separator />
                      ...
                      <div key={field.id}>
                        ...
                        <p className="font-medium">
                          {field.type === 'checkbox' ? (value ? '✓ Sì' : '✗ No') :
                           field.type === 'currency' ? `€ ${value}` : value}
                        </p>
                      </div>
                    )
                  })}
